...
const check_email = () => {
    ...
    if (email.value !== '') { //입력이 된 경우
        if (email_reg.test(email.value)) { //이메일의 형식이 맞는 경우
            document.getElementById('help_email').innerHTML = ""; //하단 설명 삭제
        } else { //이메일의 형식이 올바르지 않은 경우
            document.getElementById('help_email').innerHTML = "정확한 이메일 주소를 입력하세요."; //하단 설명 출력
